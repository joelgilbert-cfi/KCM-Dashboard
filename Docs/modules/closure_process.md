...
4. **Sending**: Confirming triggers an email via Resend (`/api/send-closure-email`) and logs the event in `closure_requests` and `closure_request_clusters`.
...
3. **Storage**: Data is saved to the `closure_tracker` table.
